...
1. **接受客户端请求**：
   - 主线程负责监听客户端连接，并将新的连接分配给多个 I/O 线程进行后续处理。
2. **I/O 线程读取请求**：
   - 一旦有新数据到达，主线程会通过事件驱动机制（如 epoll）检测到这些事件，然后将实际的数据读取任务交给 I/O 线程池中的某个线程来完成。
   - 这些 I/O 线程并行地从不同的客户端连接中读取数据，将收到的数据解析成 Redis 命令。
3. **命令队列**：
   - 每个 I/O 线程在读取并解析完客户端的命令后，会将这些命令放入一个共享的命令队列中。
   - Redis 的多线程模型设计非常注重性能，因此在实现这些同步机制时，会尽量减少锁的开销。例如，可以使用以下技术来优化：
     - **细粒度锁**：只在必要的地方加锁，尽可能缩小锁的范围，减少锁的持有时间。
     - **无锁队列**：采用无锁编程技术，如环形缓冲区（ring buffer），结合原子操作，实现高效的队列操作。
     - **批量处理**：一次性处理多个命令，减少频繁的锁操作。例如，一个 I/O 线程可以批量读取多个命令，然后统一加锁，将这些命令一次性放入队列。
4. **主线程执行命令**：
   - 主线程从命令队列中取出命令，并按顺序逐一执行。由于命令的执行是在单线程中进行，因此避免了复杂的锁管理问题，确保了命令执行的原子性和一致性。
5. **I/O 线程返回响应**：
   - 在命令执行完毕后，主线程会将结果放回对应的客户端缓冲区。然后，I/O 线程再次接管，将这些结果发送回客户端。

**为什么这种方法有效？**

- **减少上下文切换**：
  - 虽然使用了多线程进行 I/O 操作，但命令执行依然在单线程中进行，这样可以避免频繁的上下文切换，提高整体效率。
- **充分利用多核 CPU**：
  - 网络 I/O 操作通常是耗时的，通过让多个 I/O 线程并行工作，可以更好地利用多核 CPU 的优势，提升系统吞吐量。
- **简化编程模型**：
  - 保持单线程的命令执行逻辑，使得开发者不需要担心并发带来的数据一致性和死锁等问题，代码更加简洁可靠。
...
- Master 最好不要写内存快照，如果 Master 写内存快照，save 命令调度 rdbSave 函数，会阻塞主线程的 工作，当快照比较大时对性能影响是非常大的，会间断性暂停服务
- 如果数据比较重要，某个 Slave 开启 AOF 备份数据，策略设置为每秒同步一 
- 为了主从复制的速度和连接的稳定性，Master 和 Slave 最好在同一个局域网 
- 尽量避免在压力很大的主库上增加从库
- 主从复制不要用图状结构，用单向链表结构更为稳定，即:Master <- Slave1<- Slave2 <- Slave3...这样的 结构方便解决单点故障问题，实现 Slave 对 Master 的替换。如果 Master 挂了，可以立刻启用 Slave1 做 Master，其他不变。

**假如 Redis 里面有1 亿个 key，其中有10w 个 key 是以某个固定的已知的 前缀开头的，如果将它们全部找出来?**

使用 keys 指令可以扫出指定模式的 key 列表。

对方接着追问:如果这个 redis 正在给线上的业务提供服务，那使用 keys 指令会有什么问题?

这个时候你要回答 redis 关键的一个特性:redis 的单线程的。keys 指令会导致线程阻塞一段时间，线上服务会停顿，直到指令执行完毕，服务才能恢复。这个时候可以使用 scan 指令，scan 指令可以无阻塞的提取出指定模式的 key 列表，但是会有一定的重复概率，在客户端做一次去重就可以了，但是整体所花费的时间会比直接用 keys 指 令⻓。
...
如果对方追问 redis 如何实现延时队列?我估计现在你很想把面试官一棒打死如果你手上有一根棒球棍的话，怎么 问的这么详细。但是你很克制，然后神态自若的回答道:使用 sortedset，拿时间戳作为 score，消息内容作为 key 调用 zadd 来生产消息，消费者用 zrangebyscore 指令获取 N 秒之前的数据轮询进行处理。到这里，面试官暗地 里已经对你竖起了大拇指。但是他不知道的是此刻你却竖起了中指，在椅子背后。
