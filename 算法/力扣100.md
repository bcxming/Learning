...
int subarraySum(vector<int>& nums, int k) {
    ...
    for(auto num:nums){
        sum+=num;
        result+=map[sum-k];//此时的sum就是当前的边界，所以先加再更新map
        ++map[sum];//记录下前缀和
    }
    return result;
}
...
**优化二：num[i] + nums[i+1] + nums[i+2]>target后面的肯定更大**

**优化三：num[i] + nums[n-2] + nums[n-1]<target前面的肯定更小**
...
**短的那条线必须舍去，因为短线和其余的任何一条线都不能使面积增大**
...
func trap(height []int) int {
    left,right,pre_max,suf_max := 0,len(height)-1,0,0
    ans := 0
    for left<right{
        pre_max = max(pre_max,height[left])//前缀
        suf_max = max(suf_max,height[right])//后缀
      //本来应该用一个前缀数组和后缀数组的，然后用他们的最小值减去height[i],现在只要当前最大前缀小于当前最大后缀，就可以计算答案，空间复杂度从o（n）变成o（1）
        if pre_max<suf_max{
...
如果 `nums[i]` 的值为 `k`，那么 `nums[i]` 应该移动到 `nums[k-1]` 的位置上。注意，这里的 `k` 必须是正整数，并且在数组的有效范围内 `[1, n]` 内

`swap(nums[nums[i] - 1], nums[i])` 将当前元素 `nums[i]` 与它应该在的位置上的元素 `nums[nums[i] - 1]` 交换。
...
- 路径问题：dfs和vector来做，最大路径：b + a + a 的父结点+a + c + a 的父结点**(root,path)**
...
- 节点问题：层序
- 平衡问题：abs(leftvalue-rightValue)>1?-1:1+max(leftvalue,rightValue)搜索两条边
- 翻转二叉树：swap(root->left,root->right);
- 公共祖先：if (root == q || root == p || root == NULL) return root;
...
```cpp
for(int i = 0;i<nums.size()&&i<=max_count;i++){
    //不断的更新能够到的最远距离
    max_count = max(nums[i]+i,max_count);
    if(max_count>=nums.size()-1)return true;
}
```
...
public:
    string largestNumber(vector<int>& nums) {
        ...
        if(strs[strs.size()-1]=="0")return "0";
        string res;
...
递归要注意状态定义dp和状态转移方程dp方程

二叉树递归 -> 回溯 -> 记忆化搜索 -> 递推

距离问题：二维想想dp[i] [j]可以由哪几个方向推导出来dp[i] [j] = min({dp[i-1] [j],dp[i] [j-1],dp[i-1] [j-1]})+1;
...
回文子串的遍历：就是s[i]与s[j]相等，s[i]与s[j]不相等这两种
...
func minCostClimbingStairs(cost []int) int {
    ...
    f2 := 0
  	//空间复杂度为0(1)
    for i:= 2;i<=n;i++{
        new_f := min(f1+cost[i-2],f2+cost[i-1])
        f1 = f2
        f2 = new_f
    }
    return f2
...
func countTexts(pressedKeys string) int {
    ...
    for i,c :=range pressedKeys{
        cnt ++
        if i==len(pressedKeys)-1||pressedKeys[i+1]!= byte(c){//找到一个完整的字符
            if c!='7'&&c!='9'{
                ans = ans*f[cnt]%mod
            }else{
                ans = ans*g[cnt]%mod
...
func deleteAndEarn(nums []int) int {
    sum := make([]int,10005)
    for _,v := range nums{//统计每个数出现的个数
        sum[v]++
    }
  	//dp[0] = 0,dp[1] = sum[1],因为0不管多少个都是获取不点数的
    dp := []int{sum[0], max(sum[0], sum[1])}
    for i:= 2;i<len(sum);i++{
      //从i=2开始打劫，如果sum[i] = 0,那么此时结果是不会影响啥的
        dp = append(dp,max(sum[i]*i+dp[i-2],dp[i-1]))
    }
    return dp[len(sum)-1]
}
```
...
有n个物品，第i个物品的体积为w[i]，价值为v[i]，每个物品至多选一个，求体积不超过cap的最大价值和

回溯三问：

- 当前操作：第i个物品选或不选，选了剩余容量减少w[i]
- 子问题：剩余容量为c时，从前i个物品中获得的最大价值
- 下一个子问题：不选/选，从前i-1个物品中获得c-w[i]或者c的最大和
...
dfs = func(i,c int)(res int){
    ...
    defer func(){*p = res}()//更新数组
    if c<nums[i]{
        return dfs(i-1,c)//价值小于c，只能不选
    }
    return dfs(i-1,c)+dfs(i-1,c-nums[i])//选和不选加起来
}
return dfs(n-1,m)//从前i个元素中选c容量的最大值
```
...
f[0] = 1
for _, x := range nums {
  	//因为i+1用到了i，之后就不会用到i，那么就可以用一个数组来表示
    for c := m; c >= x; c-- {
        f[c] += f[c-x]
    }
}
...
有n种物品，第i种物品的体积为w[i]，价值为v[i],每种物品无限次选，体积不操过cap的最大价值和

dfs（i，c） = max（dfs（i-1，c），dfs（i，c-w[i]+v[i])

边界条件：不同的题目，所要的答案不同， 比如：方案数，最大、小值，数字个数，能否构成？ 这也就意味着 dp 数组值可以为数值，也可以是 bool类型 另外，同样是数值的情况下，不同的要求，也会造成不同的初始值 f【0】【0】：
...
bool wordBreak(string s, vector<string>& wordDict) {
    ...
    //记忆化搜索
    vector<int>memo(n,-1);
    //递推dfs(i)代表下标为i的字符串是否可以由子串组成
    //状态转移：枚举切分点k，dfs(i) = set.contains(s[k..i])&&dfs(k-1)
    //记忆化搜索，memo[i]代表已经被计算过了
    //递归终点：i<0，说明st.contains(i)已经被计算满足条件了，那么就返回true
    auto dfs = [&](auto &&dfs,int i)->bool{
        ...
        for(int k = i;k>=0;k--){
            ...
            ok = st.contains(suffix)&&dfs(dfs,k-1);
...
func maxProduct(nums []int) int {
    ...
    for i:=1;i<len(nums);i++{
        //如果num[i]是正数，这时max_num,min_sum相应的增大减小
        //如果是负数，那么这时候最小值*一个负数就摇身一变成了最大值
        //如果是0，那么就从num[i]重新开始计算
        mx,mn := max_num,min_num
        max_num = max(mx*nums[i],max(nums[i],mn*nums[i]))
        min_num = min(mn*nums[i],min(nums[i],mx*nums[i]))
        //每次更新答案
        ans = max(max_num,ans)
    }
		return ans
...
首先，我们定义一个 dp 数组，其中第 i 个元素表示**以下标为 *i* 的字符结尾**的最长有效子字符串的长度。

那么开始分解子问题：

- 如果第i个元素是“(”，此时没人和其配对，那么dp【i】 = 0
- 如果元素是“)"
  - 如果i-1是“(”,那么dp【i】 = dp【i-2】+2
  - 如果i-1是“)”,那就要看第i-dp【i-1】-1个位置是啥了，如果是“(”，那么dp【i】 = dp【i-1】+2+dp【i-dp【i-1】-2】+2
...
![image-20240728214816399](力扣100.assets/image-20240728214816399.png)
